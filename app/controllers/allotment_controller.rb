class AllotmentController < ApplicationController
  def allot
    ...
    num_to_allot_each = 50

    iterator = 0

    alum_to_allot.each do |alumnus|
      temp = TieAlumniWithStudentMember.new
      # temp.alumni_id        = alumnus.id.to_i
      temp.alumni_id        = alumnus
      ...
      if studentmember_index >= ids_to_allot.count
        ...
        puts temp.alumni_id.to_s + ' alumnus allotted to student member ' + temp.studentmember_id.to_s + ' ' + Student.find(temp.studentmember_id).name
        # temp.save!
      end
      iterator += 1
    end
